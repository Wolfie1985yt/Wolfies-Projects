...
function togglePixelNotes()
    ...
    if getPropertyFromClass('data.PlayState', 'data.data.isPixelStage') then
        setPropertyFromClass('loadState.data.PlayState', 'data.isPixelStage', false)
        ...
        setPropertyFromClass('data.PlayState', 'data.isPixelStage', true)
        for i = 0,getProperty('strumLineNotes.length')-1 do
            strumTexture = getPropertyFromGroup('strumLineNotes', i, 'texture')
            if strumTexture == nil then
                strumTexture = 'NOTE_assets'
        ...
        for i = 0,getProperty('notes.length')-1 do
            ...
            if isSustainNote then
                if string.find(getPropertyFromGroup('notes', membersIndex, 'animation.curAnim.name'), 'holdend') then
                    setPropertyFromGroup('notes', membersIndex, 'scale.y', getPropertyFromClass('data.PlayState', 'daPixelZoom'))
                    updateHitboxFromGroup('notes', membersIndex)
                    setPropertyFromGroup('notes', membersIndex, 'offsetX', 30)
                else
                    setPropertyFromGroup('notes', membersIndex, 'scale.y', (getPropertyFromGroup('notes', membersIndex, 'scale.y'))*(stepCrochet/100*1.05)*(getProperty('songSpeed'))*(1.19)*((6/getPropertyFromGroup('notes', membersIndex, 'height')))*(getPropertyFromClass('data.PlayState', 'daPixelZoom')))
                    setPropertyFromGroup('notes', membersIndex, 'offsetX', 30)
                end
                setPropertyFromGroup('notes', membersIndex, 'offsetX', 30)
            end
...
function onSpawnNote(membersIndex, noteData, noteType, isSustainNote)
    ...
    if getPropertyFromClass('data.PlayState', 'data.isPixelStage') then

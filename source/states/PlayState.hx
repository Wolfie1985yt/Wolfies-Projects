...
class PlayState extends MusicBeatState
{
	...
	public static var videoStuff:Array<Array<String>> = [
		["confessions-of-a-rotten-girl", "https://download943.mediafire.com/owsxdvf96bygNgp8Ka4lXZjm3d1XuP9pzmeY53iYQgKl5gouetbimTRpOvYWMAclTC8aCTo3PoysZMbyGusb7HEi-iGMO2nc9mnuRdXncBAG-JT3UflquSlh4_8xRCVRuItvFkAGyC24ytbI8ak_Oii_32g7OGIqFkrOI-HsOo35sA/j1baha2q8gzoqgg/confessions-of-a-rotten-girl.mp4"],
		["m@gical-cure-love-shot", "https://download1326.mediafire.com/kwykicbovpmgx2tBuYrTxfeQbgr2fTZC4ShLNauDlzDxCimOZOezM-4uMb_pVP9Pjw4o0psViRP46q6-y3djK4MkpG-MHE8iMHdgRTWL5y_xntsgQaB42FkEJZOYHyDRvoYba9vNFin_Q91DUBqwiV4YNglvhjg-q02_geqVt5lHMA/y319tnaddolz80k/m%40gical-cure-love-shot.mp4"],
		["candycookiechocolate", "https://download1654.mediafire.com/v3kpyk2d9bbgAy51X_DnH0uX8ZvrMl1R-xKmXjQTCrhNTX_t1gylytUyzoh7z7QihaR4qAYiabmbaIF153CA-oZc858CU3myZtBt4QBI8Mj0jddtzi62o3AiC5Q0kAvYlQs5PXC-lPg2uwiGFVcKaRXpi-iBZ8xKdkOKQ_gF7yB16Q/okwxtrau137r5e2/candycookiechocolate.mp4"],
		["tetoris", "https://download1591.mediafire.com/l0mbi31jq4qgyiUClbjZJLxFl6Dnh0yEFBbvmC9ie3HlUPp7oCFnepZlgVZoh2SgRoQqOaGlG3hc8fsWP7dfS-ipNvgOTF-UxEZ3nP-Ocq-qIYocVS_LdPn_Gp4f-rwJGsE3MrbpU-cwRIIK7Me_sXpRMv2FvPdYmP5XFZS20DMeiA/bidvk1l9flp73cv/tetoris.mp4"]
	];
	...
	public var boyfriendUnderlayMap:Map<String, Character> = new Map<String, Character>();
	...
	#if VIDEOS_ALLOWED
	public var video:VideoSprite = null;
	#end

	public var boyfriendGroup:FlxSpriteGroup;
	public var boyfriendUnderlayGroup:FlxSpriteGroup;
	...
	public var boyfriendUnderlay:Character = null;
	...
	#if VIDEOS_ALLOWED
	public function preloadVideo(videoName:String):Void
	{
		var videoPath = Paths.video(videoName);
		if (!FileSystem.exists(videoPath)) {
			trace('Video not found: ' + videoPath);
			return;
		} else {
			var videoPre:VideoSprite = new VideoSprite(videoPath, false, false, false);
			videoPre.play();
			videoPre.cameras = [camVideo];
			add(videoPre);
			videoPre.pause();
			new FlxTimer().start(1, function(tmr:FlxTimer) {
				remove(videoPre);
			});
			trace('Video Preloaded Successfully: ' + videoPath);
		}
	}
	#end
	...
	function downloadFile(url:String, savePath:String):Void {
		...
		http.onBytes = function(bytes:Bytes) {
			try {
				...
				trace('File downloaded and saved to ' + savePath);
			} catch (e:Dynamic) {
				trace('Failed to save file: ' + e);
			}
		};
	...
	override public function create()
	{
		...
		#if VIDEOS_ALLOWED
		for (i in 0...videoStuff.length) {
			if (videoStuff[i][0].toLowerCase() == songName.toLowerCase() && !FileSystem.exists("assets/videos/" + songName.toLowerCase() + ".mp4")) {
				downloadFile(videoStuff[i][1], 'assets/videos/' + videoStuff[i][0].toLowerCase() + ".mp4");
				trace("file downloaded: assets/videos/" + videoStuff[i][0].toLowerCase() + ".mp4");
			}
		}
		#end
		...
		boyfriendUnderlayGroup = new FlxSpriteGroup(BF_X, BF_Y);
		...
		switch(songName.toLowerCase()) {
			case 'tv-time':
				boyfriendUnderlay = new Character(0, 0, 'kris-guitar-outline', true);
				startCharacterPos(boyfriendUnderlay);
				boyfriendUnderlayGroup.add(boyfriendUnderlay);
			default:
				boyfriendUnderlay = new Character(0, 0, SONG.player1, true);
				startCharacterPos(boyfriendUnderlay);
				boyfriendUnderlayGroup.add(boyfriendUnderlay);
				boyfriendUnderlayGroup.alpha = 0;
				boyfriendUnderlayGroup.visible = false;
		}
		...
		startCharacterScripts(boyfriendUnderlay.curCharacter);
		...
		#if VIDEOS_ALLOWED
		var videoPath = Paths.video(songName);
		if (!FileSystem.exists(videoPath)) {
			trace('Video not found: ' + videoPath);
		} else {
			video = new VideoSprite(videoPath, false, false, false);
			video.play();
			video.pause();
			add(video);
			video.cameras = [camVideo];
			insert(members.indexOf(strumLineNotes), video);
		...
		#if LUA_ALLOWED
		if (video != null) {
			startLuasNamed('data/candycookiechocolate/stuff.lua');
			startLuasNamed('data/candycookiechocolate/healthbar.lua');
		}
		#end
		...
		if(songName.toLowerCase() == 'tv-time') {
		...
		if(songName.toLowerCase() == 'tv-time') {
			for(i in 0...4) {
				trace(strumLineNotes.members[i].x, strumLineNotes.members[i+4].x);
				var temp:Float = strumLineNotes.members[i].x + 330;
				strumLineNotes.members[i].x = strumLineNotes.members[i+4].x + 330;
				strumLineNotes.members[i+4].x = temp;
				trace(strumLineNotes.members[i].x, strumLineNotes.members[i+4].x);

				strumLineNotes.members[i].scrollFactor.x = strumLineNotes.members[i+4].scrollFactor.x = 
				strumLineNotes.members[i].scrollFactor.y = strumLineNotes.members[i+4].scrollFactor.y = 1;

				strumLineNotes.members[i].y += 50;
				strumLineNotes.members[i+4].y += 400;
			}
		}
	...
	static public function resizeGame(width:Int, height:Int, resize:Bool) {
		if(!resize) {
			...
			window.x = Math.floor((Capabilities.screenResolutionX / 2) - (width / 2));
			window.y = Math.floor((Capabilities.screenResolutionY / 2) - (height / 2));
		} else {
			FlxG.fullscreen = false;
			FlxG.resizeWindow(width, height);
			FlxG.scaleMode = new RatioScaleMode(true);
	...
	{
		insert(members.indexOf(boyfriendUnderlayGroup), obj);
	}
	...
	private function generateSong():Void
	{
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				if(songName.toLowerCase() == 'tv-time')
					swagNote.scrollFactor.x = swagNote.scrollFactor.y = 1;
				...
				if(roundSus > 0)
				{
					for (susNote in 0...roundSus)
					{
						...
						if(songName.toLowerCase() == 'tv-time')
							sustainNote.scrollFactor.x = sustainNote.scrollFactor.y = 1;
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				boyfriendUnderlay.stunned = true;
				deathCounter++;
		...
		switch(eventName) {
			case 'Hey!':
				...
				if(value != 1) {
					...
					boyfriendUnderlay.playAnim('hey', true);
					boyfriend.specialAnim = true;
					boyfriendUnderlay.specialAnim = true;
					boyfriend.heyTimer = flValue2;
					boyfriendUnderlay.heyTimer = flValue2;
				}
		...
		if(!note.hitCausesMiss) //Common notes
		{
			if(!note.noAnimation)
			{
				...
				if(char != null)
				{
					...
					if(canPlay) {
						char.playAnim(animToPlay, true);
						boyfriendUnderlay.playAnim(animToPlay, true);
					}
					char.holdTimer = 0;
					boyfriendUnderlay.holdTimer = 0;
	...
	override function stepHit()
	{
		...
		switch(curStep) {
			case 128:
				if(songName.toLowerCase() == 'tv-time') {
					for(i in 0...4) {
						FlxTween.tween(strumLineNotes.members[i], {alpha: 1}, 2, {ease: FlxEase.smootherStepOut});
						FlxTween.tween(strumLineNotes.members[i+4], {alpha: 1}, 2, {ease: FlxEase.smootherStepOut});
					}
				}
		}
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.data.camZooms && autoZooming)
			{

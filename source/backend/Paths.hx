...
class Paths
{
	...
	public static function clearUnusedMemory()
	{
		for (key in currentTrackedAssets.keys())
		{
			if (!localTrackedAssets.contains(key) && !dumpExclusions.contains(key))
			{
				...
				debug.FPSCounter.untrackVRAM(key);
			}
		}
		System.gc();
	}
	...
	public static function getPath(file:String, ?type:AssetType = TEXT, ?parentfolder:String, ?modsAllowed:Bool = true):String
	{
		...
		if (parentfolder == 'mods') {
			var modFile = getFolderPath(file, parentfolder);
			modFile = 'mods/' + modFile.substr(12, modFile.length);
			return modFile;
		...
		if(OpenFlAssets.exists("mods/" + file + type))
			return "mods/" + file + type;
	...
	inline public static function getSharedPath(file:String = '') {
		if(file.substr(0, 21) == 'images/assets/shared/') {
			file = file.substr(21,file.length);
			return getSharedPath(file);
		}
	...
	public static function cacheBitmap(key:String, ?parentFolder:String = null, ?bitmap:BitmapData, ?allowGPU:Bool = true):FlxGraphic
		{
			if (bitmap == null)
			{
				...
				if (bitmap == null)
				{
					trace('Sprite Not Found: ' + getPath(key, IMAGE, parentFolder));
					return null;
				}
			}
			...
			{
				trace('Sprite Loaded in RAM: ' + getPath(key, IMAGE, parentFolder));
			}
